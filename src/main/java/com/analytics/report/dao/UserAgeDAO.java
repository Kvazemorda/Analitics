...
import com.analytics.report.entity.report.UserAge;
...
public class UserAgeDAO {
    ...
    public ArrayList<UserAge> userAgesList(QueryClient queryClient){
        ArrayList<UserAge> list = new ArrayList<>();
        URI url2 = UriComponentsBuilder.fromUriString("https://api-metrika.yandex.ru/stat/v1/data/")
                ...
                .queryParam("dimensions", "ym:s:ageInterval")
        ...
        for(int j = 0; j < dimensionDatas.size(); j++){
            ...
            list.add(new UserAge(dimensionDatas.get(j).getDimensions().get(0).getName(), metric, conversation));
        }
